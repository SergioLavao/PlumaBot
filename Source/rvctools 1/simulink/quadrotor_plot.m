...
function sys = mdlOutputs(t,u,s, plot, enable, quad)
    ...
    if enable == 1
        ...
        if length(s) == 1
            ...
            axis([-5 15 -15 5 0 10]);
            % Adding rice-crop surface
            %img = imread('crop.jpg');     % Load a sample image
            %xImage = [-5 -5; 10 10];   % The x data for the image corners
            %zImage = [0 0; 0 0];             % The y data for the image corners
            %yImage = [-5 -15; -5 -15];   % The z data for the image corners
